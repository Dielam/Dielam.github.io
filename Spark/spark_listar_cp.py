...
if len(sys.argv) <= 1:
    ...
    if len(sys.argv[1]) > 5:
        ...
        #Spark configuration
        conf = SparkConf().setAppName('ListCP')
        ...
        localidadRDD = localidadRDD.filter(lambda rows: wanted == rows[6])
        ...
        # map(CP,Localidad,Empresa,precio_gasolina_95,precio_gasoleo_a,precio_gasoleo_b,precio_bioetanol,precio_nuevo_gasoleo_a,precio_biodiesel,f__ester_metilico,f__bioalcohol,precio_gasolina_98,precio_gas_natural_comprimido,precio_gas_natural_licuado,precio_gases_licuados_del_petr)
        result = datosRDD.map(lambda rows: ([rows[2], rows[1], rows[19], float(rows[7]), float(rows[8]), float(rows[9]), float(rows[10]), float(rows[11]), float(rows[12]), float(rows[13]), float(rows[14]), float(rows[15]), float(rows[16]), float(rows[17]), float(rows[18])]))
