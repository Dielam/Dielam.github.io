...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[11]))) #map (CP, precio_gasolina_95)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[12]))) #map (CP, precio_gasoleo_a)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[13]))) #map (CP, precio_gasoleo_b)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[14]))) #map (CP, precio_bioetanol)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[15]))) #map (CP, precio_nuevo_gasoleo_a)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[16]))) #map (CP, precio_biodiesel)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[17]))) #map (CP, precio_ester_metilico)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[18]))) #map (CP, bioalcohol)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[19]))) #map (CP, precio_gasolina_98)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[20]))) #map (CP, precio_gas_natural_comprimido)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[21]))) #map (CP, precio_gas_natural_licuado)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
...
if data.isEmpty():
    ...
    precio = data.map(lambda line: (str(line.split(',')[23]),float(line.split(',')[22]))) #map (CP, precio_gases_licuados_del_petr)
    count = data.map(lambda line: (str(line.split(',')[23]), 1)) #map (CP, contador)
    aggreg1 = precio.reduceByKey(lambda a, b: a+b)
    aggreg2 = count.reduceByKey(lambda a, b: a+b)
    union = aggreg1.join(aggreg2)
